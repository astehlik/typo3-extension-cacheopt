...
TYPO3 core's auto-tagging (see above) only caps the page cache lifetime for
records that are actually rendered. A record that is currently hidden due to
a future starttime is never rendered, so its starttime cannot be taken into
...
candidates with a future starttime and cap the page cache lifetime
accordingly, without letting the hidden content itself reach the rendered
output.
